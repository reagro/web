...
Unfortunately, R does not have an easy platform for such a specific model, and we thus need to install a package made for this approach: "mvprobit". For purposes of simplicity in demonstrating the techique, we will reduce the number of independent and dependent variables from those used in the paper. Here, we show three simultaneous probit models, with dependent SIPs diversity, notill, and improvar, and just seven independent variables. 
