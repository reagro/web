...
In this chapter we analyze rice crop cut data from Odisha, collected by Iftikar et al., 2017:
...
```{r f30}
df <- na.omit(df[, c("Year", fert[1:3], "yha")])
df$fert <- rowSums(df[, fert[1:3]])
plot(yha~fert, data=df)
m <- lm(yha ~ Year + fert, data=df)
summary(m)

mr <- randomForest::randomForest(yha ~ Year + DAP_Total + MoP_Total + Urea_Total, data=df)
mr
randomForest::partialPlot(mr, df, x.var="Urea_Total")


mrs <- randomForest::randomForest(yha ~ Year + fert, data=df)
mrs
randomForest::partialPlot(mrs, df, x.var="fert")
