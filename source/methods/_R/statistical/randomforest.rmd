...
For this chapter you need the following *R* packages: `randomForest`, `rpart`, `reagro`, and `predicts`. They are all available for download from CRAN [see these instructions](/installation.html) about installing *R* packages.
...
summary(lrm)
predicts::RMSE_null(d2$SOC, predict(lrm, d2))
```
...
predicts::RMSE_null(d2$SOC, predict(cart, d2))
```
...
predicts::RMSE_null(d1$SOC, predict(cart2, d1))
```

That is very similar to the result for `d1` (`r round(predicts::RMSE_null(d2$SOC, predict(cart, d2)), 2)`).
...
predicts::RMSE_null(d2$SOC, predict(cart, d2))
# model 1, train data
predicts::RMSE_null(d1$SOC, predict(cart, d1))
# model 2, test data
predicts::RMSE_null(d1$SOC, predict(cart2, d1))
# model 2, train data
predicts::RMSE_null(d2$SOC, predict(cart2, d2))
```
...
for (i in 1:n) {
	...
	eval[i] <- predicts::RMSE_null(d2$SOC, p)
	predictions[, i] <- p
}
```
...
```{r randfor_150}
mean(eval)
predicts::RMSE_null(d2$SOC, pavg)
```
...
```{r randfor_180}
p <- predict(rf, d2)
predicts::RMSE_null(d2$SOC, p)
```

```{r randfor_190}
# Mean of squared residuals
predicts::RMSE(d2$SOC, p)^2
# % Var explained:
...
for (i in 1:n) {
	...
	p <- predict(m, test)
	rfRMSE[i] <- predicts::RMSE(test$SOC, p)
	rfRMSE_null[i] <- predicts::RMSE_null(test$SOC, p)
...
for (i in 1:n) {
	...
	p <- predict(m, test)
	lmRMSE[i] <- predicts::RMSE(test$SOC, p)
	lmRMSE_null[i] <- predicts::RMSE_null(test$SOC, p)
...
vi <- predicts::varImportance(rfm, d, predvars)

vimean <- colMeans(vi)
p <- predict(m, d)
RMSEfull <- predicts::RMSE(d$SOC, p)
...
```{r randfor_260}
mlr <- lm(model, data=d)
vi <- predicts::varImportance(mlr, d, predvars)
vimean <- colMeans(vi)
p <- predict(m, d)
RMSEfull <- predicts::RMSE(d$SOC, p)
x <- sort(vimean - RMSEfull)
...
```{r randfor_290}
pr_pH <- predicts::partialResponse(rfm, d, "pH")
plot(pr_pH, type="l")
...
```{r randfor_300}
lrm <- lm(model, data=d)
pr_pH <- predicts::partialResponse(lrm, d, "pH")
plot(pr_pH, type="l")
rug(quantile(d$pH, seq(0, 1, 0.1)))
```
