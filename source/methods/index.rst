...
The section on `spatial modeling <spatial>`__ builds on the statistical modeling section to discuss predicting spatial patterns. It describes methods that take point based observations (e.g. from soil samples or household surveys) to estimate values at any location trough interpolation, spatial distribution modeling, or hybrid approaches. It continues with a chapter on the use of satellite remote sensing data; and ends with a chapter on downscaling methods to estimate higher resolution data.

The last section is about `mechanistic models <mechanistic>`__ models. The mathematical form of these models is based on understanding of the processes of interest. These include relatively simple models such as the QUEFTS model to assess soil fertility and rather complex dynamic models to simulate crop growth or a crop disease.
