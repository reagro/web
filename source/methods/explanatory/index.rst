...
The first chapter introduces the WOFOST dynamic crop growth simulation model. Crop growth simulation models provide a quantitative description of the mechanisms that cause the behavior of a system of interest (a system is a limited part of reality that contains interrelated elements). They are often referred to as *mechanistic* and *explanatory* because the model developer uses known mechanisms sub-system processes, such as leaf-level photosynthesis, to construct a model of the system of interest, such as crop growth. To make best use of crop simulation in a meaningful way, it is important to have some general knowledge of crop ecology. You may need to consult background texts in crop ecology and crop modeling [insert references]. 
...
We explains the general use of `WOFOST <wofost/index.html>`__ and in the chapter on spatial modeling we show how you can use WOFOST `with spatial data <wofost-spatial.html>`_. Stay tuned for updates on LINTUL, APSIM, and other crop growth simulation models. 

The next chapters discuss the use of `QUEFTS`. QUEFTS is a static rule-based model to estimate crop yield given a few soil properties and the amount of fertilizer applied, or to estimate the amount of fertilizer needed to reach a particular yield. First, we provide an `introduction to the basic ideas <quefts.html>`__ of the model, and the model input and output. In the next chapter we show how to use QUEFTS with spatial data on soil properties to make `predictions for regions <quefts_spatial.html>`__.

Still coming: disease modeling
