...
---
#Rice survey 2106 (data analysis after including Rainfall (CHIRPS)) CART, BART & RandomForest 
...
```{r}
rice_npl <- read.csv("CART.csv", header=T)
rice_npl_rf <- read.csv("Rice2016_rf15.csv", header=T)

#load package dplyr for data management
#library(dplyr)
#Merge weather data with other data
rice_rf <- merge (rice_npl, rice_npl_rf)

#First manage the soil related variables before merging with rice_rf df 
rice_soil <- read.csv("Rice_survey_soil_data_extracted_soilgrids.csv", header=T)
ls(rice_soil)
...
rows <- sample(x=nrow(rice_rf1), size=0.8 * nrow(rice_rf1))
train <- rice_rf1[rows,]
test <- rice_rf1[-rows,]

##Creating basic random forest (1)
library(randomForest)
## randomForest 4.6-7
## Type rfNews() to see new features/changes/bug fixes.
set.seed(123457)

model0 <- randomForest(s_yield~., data=train, importance=TRUE, maxnodes=10, mtry=5, ntree=1000,type=regression, na.action=na.omit)
varImpPlot(model0, type=1)
```
...
## Creating final random forest (2)
...
par(mfrow=c(2,2))
for (v in vars[1:4]) {
  partialPlot(model1, rice_rf1, c(v), ylab = "Yield", main=v)
}
