...
Here we show a thresholding based approach to map rice using [Sentinel-1SAR C-band Synthetic Aperture Radar data](https://sentinel.esa.int/web/sentinel/user-guides/sentinel-1-sar/). To learn more about the data that is available through [Google Earth Engine](https://developers.google.com/earth-engine/) read the [data description page](https://developers.google.com/earth-engine/datasets/catalog/COPERNICUS_S1_GRD).
...
Repeat the rice mapping exercise for a different year with `VH` polarization data.
