...
library(reagrodata)
ce_income(rice_income, rho=1.5)
```

And with insurance it would be
```{r ins212, message=FALSE}
ce_income(total_income, rho=1.5)
```
...
```{r ins40}
rho <- seq(0,3,.1)
ce_rice <- sapply(rho, function(r) ce_income(rice_income, rho=r))
ce_tot <- sapply(rho, function(r) ce_income(total_income, rho=r))
...
hh <- aggregate(z[, c("income", "income_with_ins")], z[,"fid", drop=FALSE], function(i) ce_income(i, 1.5))
