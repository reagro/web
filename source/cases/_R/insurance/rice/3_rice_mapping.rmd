...
ff <- agrodata::data_rice("MODIS", vidir)
```
...
fevi  <- luna::modisDate(fevi)
fndfi <- luna::modisDate(fndfi)
...
# extract time series 
ts_evi <- extract(evi, s, drop = TRUE)[,-1]
ts_fevi <- extract(fevi, s, drop = TRUE)[,-1]
ts_fndfi <- extract(fndfi, s, drop = TRUE)[,-1]
...
Below we plot phenology (EVI) of each sampling site at a time. We find this is to be one of the best way to find `phenorice` parameters. Notice the use of `par(ask)` to control plotting.

```{r phenoplot_ask, eval = FALSE, include=FALSE}
par(ask=TRUE)
for(i in 1:nrow(ts_evi)){
  # raw EVI
  plot(dates, ts_evi[i,], pch = 16, col = "red", cex = 0.8, ylim = c(0,0.7),
...
ts_fevi <- as.matrix(ts_fevi)
ts_fndfi <- as.matrix(ts_fndfi)
