...
phenorice <- function(evi, ndfi, p, lst,...){
  ...
  dates <- .checkEVIaftermax2(dates, evi, p$windecr, p$decr1)
  ...
  if (nrow(dates) == 0) {
    ...
    # Harvest date
    eos <- min((dates[2]:length(evi))[evi[dates[2]:length(evi)] <= (evi[dates[2]] - (p$decr2*(evi[dates[2]]-evi[dates[1]])))])
    # temporarily removed season length condition: & (eos-dates[1] <= p$tl2)
    if((eos-dates[1] >= p$tl1)){
...
getPars <- function() {
  pars <- list()
  pars$evi_meanth <- 0.5 # threshold for annual mean EVI
  pars$evi_maxth <- 0.4 # threshold for maximum evi 
  pars$evi_minth <- 0.3 # threshold for minimum evi 
  pars$pos_start <- 20 # start of heading, after December
  pars$pos_end <- 40 # end of heading, not possible after April
  pars$vl1 <- 5 # the shortest vegetative growth length
  pars$vl2 <- 20 # the longest vegetative growth length
  pars$winfl <- 3 # period for flooding around min EVI
  pars$minndfi <- -0.1 # threshold for ndfi
  pars$windecr <- 15 # period after EVI maximum 
  pars$decr1 <- 0.4 # percent decrease of EVI after EVI maximum for EVI change ratecheck
  pars$decr2 <- 0.5 # percent decrease of EVI after EVI maximum for End of Season Check
  pars$tl1 <- 8 # the shortest total growing length
  pars$tl2 <- 20 # the longest total growing length
