...
Market access is [(in part)](https://doi.org/10.1016/j.worlddev.2012.06.004) a function of distance to market centers and transport infrastructure. Prices of outputs may be more volatile as the distance to market increases [(Moctar et al., 2015)](#references) and inadequate road infrastructure increases the costs of transportation for smallholder farmers [(Obare, 2003)](#references)
...
For this chapter you need the following `R` packages: `terra`, `geodata`, and `reagrodata`. See [these instructions](/installation.html) about installing these packages.
...
```{r access30, message=FALSE, warning=FALSE}
...
```{r access_processOSM, eval=FALSE}
...
```{r access_simplr, eval=FALSE}
library(rmapshaper)
roads <- ms_simplify(roads, keep=0.01)
```
The result was put in the reagrodata package, and that is where we get it from
...
We rasterize the roads. The values of the rasters reflect the travel costs (in min/m) of moving through a cell by road (if there is one). We use the slope layer as template for the rasterization.

The values of the final 
