...
The ID is "hdl:11529/10548190" and we can download the data with the "agdasci" package (under development). Install the package like this:


```{r}  
if (!require(agro)) devtools::install_github("reagro/agro")
```
...
```{r}  
ff <- agro::get_data_from_uri("https://data.cimmyt.org/dataset.xhtml?persistentId=hdl:11529/10548190", ".")
ff
```

We have two "gnu zip" files. Unzip these
