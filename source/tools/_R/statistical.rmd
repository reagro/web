...
```{r, echo=FALSE}
d = data.frame(variable=c('yield', 'N', 'P', 'K', 'temp', 'precip', 'ExchP', 'TotK', 'ExchAl', 'TotN', 'sand', 'clay', 'SOC', 'pH', 'AWC'),
...
Suppose we were interested in estimating soil organic carbon (SOC) across locations in West Africa. Suppose we have a lot of data on soil pH and the fraction of the soil that is sand (which are both cheap and easy to measure), and precipitation data as well (which is available for any location). Can we build a model that predicts SOC from pH, sand and precipitation? 

```{r}
par(mfrow=c(2,2), mai=rep(0.5, 4))
for (v in c("SOC", "pH", "precip", "sand")) {
	boxplot(d[,v], main=v)
}
```
...
```{r}
lrm <- lm(SOC~pH+precip+sand, data=d1)
summary(m)
rmsenull(d2$SOC, predict(lrm, d2))
```

We see that all the predictor variables are highly significant, but that the R^2^ is low. Perhaps we could improve the model by including interaction terms, but let's not go down that path here. 
...
```{r}
library(rpart)
cart <- rpart(SOC~pH+precip+sand, data=d1)
...
The big numbers express the remaining deviance. The Null deviance is 40810.6, but after the first split is has gone down to (19693.6 + 9037.7) = 28731.3.
...
```{r}
cart2 <- rpart(SOC~pH+precip+sand, data=d2)
...
for (i in 1:n) {
	...
	cartmod <- rpart(SOC~pH+precip+sand, data=d1[k,])
...
rf <- randomForest(SOC~pH+precip+sand, data=d1)
rf
```	
...
In cross-validation the data is divided into groups. Typically into 5 or 10 groups. 
...
for (i in 1:n) {
	test <- d[k==i, ]
	train <- d[k!=i, ]
	m <- randomForest(SOC~pH+precip+sand, data=train)
	p <- predict(m, test)
...
for (i in 1:n) {
	test <- d[k==i, ]
	train <- d[k!=i, ]
	m <- lm(SOC~pH+precip+sand, data=train)
	p <- predict(m, test)
...
Purpose: 
Get a sense of quality. 
Compare models to select or to average models. 
Optimize models (parameters such as ntry, nodesize)
...
```{r}
m <- randomForest(SOC~pH+precip+sand, data=d)
varImpPlot(m)
```

rf <- randomForest(SOC~pH+precip+sand, data=d)
