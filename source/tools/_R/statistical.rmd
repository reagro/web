...
```{r}
filename <- file.path(dirname(tempdir()), "soilfert.rds")
if (!file.exists(filename)) {
	download.file("http://gfc.ucdavis.edu/events/ifpri/data/soilfert.rds",  filename, mode="wb")
}
d <- readRDS(filename)
dim(d)
head(d)
...
```{r, echo=FALSE}
dtab <- data.frame(variable=c('yield', 'temp', 'precip', 'ExchP', 'TotK', 'ExchAl', 'TotN', 'sand', 'clay', 'SOC', 'pH', 'AWC', 'fert'),
description=c('crop yield (kg/ha)', 'Average temperature', 'Annual precipitation', 'Soil exchangeble P', 'Soil total K', 'Soil exchangeble Al', 'Soil total N', 'Soil franction sand (%)', 'Soil fraction clay (%)', 'Soil organic carbon (g/kg)', 'Soil pH', 'Soil water holding capacity', 'fertilizer (index) kg/ha'))
kable(dtab)
```
