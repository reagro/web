...
For this chapter you need the following *R* packages: `randomForest`, `rpart`, `reagro`, and `dismo`. They are all available for download from CRAN [see these instructions](/installation.html) about installing *R* packages.
...
summary(lrm)
reagro::RMSE_null(d2$SOC, predict(lrm, d2))
```
...
reagro::RMSE_null(d2$SOC, predict(cart, d2))
```
...
reagro::RMSE_null(d1$SOC, predict(cart2, d1))
```

That is very similar to the result for `d1` (`r round(reagro::RMSE_null(d2$SOC, predict(cart, d2)), 2)`).
...
reagro::RMSE_null(d2$SOC, predict(cart, d2))
# model 1, train data
reagro::RMSE_null(d1$SOC, predict(cart, d1))
# model 2, test data
reagro::RMSE_null(d1$SOC, predict(cart2, d1))
# model 2, train data
reagro::RMSE_null(d2$SOC, predict(cart2, d2))
```
...
for (i in 1:n) {
	...
	eval[i] <- reagro::RMSE_null(d2$SOC, p)
	predictions[, i] <- p
}
```
...
```{r}
p <- predict(rf, d2)
reagro::RMSE_null(d2$SOC, p)
```

```{r}
# Mean of squared residuals
reagro::RMSE(d2$SOC, p)^2
# % Var explained:
...
for (i in 1:n) {
	...
	p <- predict(m, test)
	rfRMSE[i] <- reagro::RMSE(test$SOC, p)
	rfRMSE_null[i] <- reagro::RMSE_null(test$SOC, p)
...
for (i in 1:n) {
	...
	p <- predict(m, test)
	lmRMSE[i] <- reagro::RMSE(test$SOC, p)
	lmRMSE_null[i] <- reagro::RMSE_null(test$SOC, p)
